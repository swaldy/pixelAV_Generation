...
/* 10 ps time slice version (NCRRC=400, samptim=4000ps -> stimstp=10ps) */
...
/* If no seedfile, use single seed and set-up generator */
...
/* read-in 25 ints and restore generator to previous state */
	...
	/* track travels in the E-field direction in the unflipped coordinate system */
		...
		vect[0] = 4.*xsize * (rvec[0] - 0.5) + xsize/2. + (vect[2] - thick/2.)*locdir[0]/locdir[2];
		vect[1] = 4.*ysize * (rvec[1] - 0.5) + ysize/2. + (vect[2] - thick/2.)*locdir[1]/locdir[2];
